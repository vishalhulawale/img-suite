...
def _compress_to_target(img: Image.Image, target_bytes: int, out_format: str) -> io.BytesIO:
    ...
    # If minimum quality still exceeds target, scale down resolution iteratively
    if best_buf.getbuffer().nbytes > target_bytes:
        scale = 0.95
        tmp = img.copy()
        for _ in range(20):
            new_w = max(1, int(tmp.width * scale))
            ...
            tmp = img.resize((new_w, new_h), Image.LANCZOS)
            buf = io.BytesIO()
            tmp.save(buf, format=out_format, quality=lo, optimize=True)
            if buf.tell() <= target_bytes:
                best_buf = buf
                break
            scale *= 0.95
